...
export class WebDAVExplorerView extends View {
    ...
    private showLoadingState(fileList: HTMLElement): void {
        const loadingItem = fileList.createEl('div', {cls: 'webdav-file-item loading'});
        const iconSpan = loadingItem.createSpan({cls: 'webdav-file-icon'});
        setIcon(iconSpan, 'loader');
        loadingItem.createSpan({cls: 'webdav-file-name', text: i18n.t.view.loading || '加载中...'});
    }

    private renderEmptyDirectory(fileList: HTMLElement): void {
        const emptyItem = fileList.createEl('div', {cls: 'webdav-file-item webdav-empty'});
        const iconSpan = emptyItem.createSpan({cls: 'webdav-file-icon'});
    ...
    private renderFileItem(fileList: HTMLElement, file: FileStat): void {
        const item = fileList.createEl('div', {cls: 'webdav-file-item is-clickable'});
        const iconSpan = item.createSpan({cls: 'webdav-file-icon'});
    ...
    private createUpDirectoryItem(fileList: HTMLElement): void {
        ...
        const iconSpan = upItem.createSpan({cls: 'webdav-file-icon'});
    ...
    private updateInteractiveElementsState(loading: boolean): void {
        // 更新文件夹项状态
        const folderItems = this.contentEl?.querySelectorAll('.webdav-file-item.folder') || [];
    ...
    private updateBreadcrumb(): void {
        ...
        parts.forEach((part, index) => {
            if (index > 0) {
                const separator = breadcrumbEl.createEl('span', {cls: 'webdav-breadcrumb-separator'});
            ...
            if (part.name === 'root') {
                ...
                const rootLink = item.createEl('a', {cls: 'webdav-breadcrumb-root-link'});
